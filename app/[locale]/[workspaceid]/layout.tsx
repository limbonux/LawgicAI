...
export default function WorkspaceLayout({ children }: WorkspaceLayoutProps) {
  ...
  // تعريف `fetchWorkspaceData` باستخدام useCallback
    ...
    try {
      ...
      for (const assistant of assistantData.assistants) {
        ...
        if (url) {
          ...
          setAssistantImages((prev) => [
            ...prev,
            { assistantId: assistant.id, path: assistant.image_path, base64: "", url: url || "" },
          ]);
        }
      }

      setChats(await getChatsByWorkspaceId(workspaceId));
      setCollections((await getCollectionWorkspacesByWorkspaceId(workspaceId)).collections);
      ...
      setChatSettings({
        model: (searchParams.get("model") || workspace?.default_model || "gpt-4-1106-preview") as LLMID,
  ...
  }, [searchParams, setChatSettings, setSelectedWorkspace, setAssistants, setAssistantImages, setChats, setCollections, setFolders, setFiles, setPresets, setPrompts, setTools, setModels]);
  ...
  }, [workspaceId, fetchWorkspaceData, router]);
